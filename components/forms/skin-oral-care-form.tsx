...
export function SkinOralCareForm({ onSubmit, onCancel }: SkinOralCareFormProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* 1. ヘッダー - 固定（shrink-0） */}
      <div className="shrink-0 border-b bg-gradient-to-r from-teal-50 to-cyan-50 px-6 py-4">
        <h2 className="text-2xl font-bold text-gray-800">🧴 皮膚・口腔ケア記録</h2>
      </div>

      {/* 2. スクロール可能なコンテンツ（flex-1 overflow-y-auto） */}
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <form id="skin-oral-care-form" onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="shrink-0 border-t bg-white/95 backdrop-blur-sm px-6 py-4 flex gap-3 justify-end shadow-lg">
        ...
        <Button type="submit" form="skin-oral-care-form">
          保存
        </Button>
      </div>
    </div>
