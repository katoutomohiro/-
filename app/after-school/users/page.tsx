...
export default function AfterSchoolUsersPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      ...
      <UserManagementModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setEditingUserId(null); const all = DataStorageService.getAllUserProfiles(); setUsers(all.filter(u => u.serviceType === "after-school")) }}
        userId={editingUserId || undefined}
        onSaved={(u) => { const all = DataStorageService.getAllUserProfiles(); setUsers(all.filter(us => us.serviceType === "after-school")) }}
      />
    </div>
  )
}
