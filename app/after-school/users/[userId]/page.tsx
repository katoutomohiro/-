...
export default function AfterSchoolUserPage() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <h1 className="text-3xl font-bold text-foreground tracking-tight">{user.name} - ケース記録・サービス実績表</h1>
              <p className="text-muted-foreground font-medium">放課後等デイサービス</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => router.push("/after-school/users")}>一覧に戻る</Button>
            </div>
          </div>
        </div>
      </header>
          ...
          <TabsContent value="single">
            <Card className="mb-4">
              ...
              <CardContent>
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-lg font-semibold">{format(selectedDate, "yyyy年MM月dd日（E）", { locale: ja })}</div>
                  <Button
                    variant="outline"
                    onClick={() => {
                      const newDate = new Date(selectedDate)
                  ...
                  <Calendar mode="single" selected={selectedDate} onSelect={(date) => date && setSelectedDate(date)} locale={ja} />
                </div>
              </CardContent>
            </Card>
            ...
            {caseRecord && (
              <CaseRecordSheet
                record={caseRecord}
                user={user}
                onSave={(updatedRecord: CaseRecord) => {
